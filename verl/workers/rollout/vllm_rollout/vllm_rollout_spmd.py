...
class vLLMRollout(BaseRollout):
    def __init__(self, model_path: str, config: DictConfig, tokenizer, model_hf_config, **kwargs):
        ...
        for k in config.keys():
            if hasattr(SamplingParams(), str(k)) and k != "seed":
                v = config.get(k)
                if v is not None:  # skip None values; let vllm use its own defaults
                    kwargs[k] = v
    ...
    def generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        if not do_sample:
            ...
            # TODO: try **
            # top_k=None means the model doesn't use it (e.g. Llama-3.2);
            # vllm requires -1 to disable top_k filtering, not None.
            top_k_val = self.config.val_kwargs.top_k
            kwargs = {
                "top_k": top_k_val if top_k_val is not None else -1,
