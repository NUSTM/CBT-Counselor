            ...
            try:
                if fsdp_version(self.actor_module_fsdp) > 0:
                    ...
                    if dist.get_rank() == 0:
                        ...
                        # Manually build full adapter_config to avoid asdict serialization issues
                        peft_cfg = peft_model.peft_config.get("default")
                        
                        adapter_config = {
                            "task_type": "CAUSAL_LM",
                            "peft_type": "LORA",
                            "auto_mapping": None,
                            "base_model_name_or_path": self.config.model.path,
                            "revision": None,
                            "inference_mode": False,
                            "r": peft_cfg.r,
                            "target_modules": list(peft_cfg.target_modules) if peft_cfg.target_modules else [],
                            "exclude_modules": list(peft_cfg.exclude_modules) if peft_cfg.exclude_modules else None,
                            "lora_alpha": peft_cfg.lora_alpha,
                            "lora_dropout": getattr(peft_cfg, 'lora_dropout', 0.0),
                            "fan_in_fan_out": getattr(peft_cfg, 'fan_in_fan_out', False),
                            "bias": getattr(peft_cfg, 'bias', "none"),
                            "use_rslora": getattr(peft_cfg, 'use_rslora', False),
                        ...
                        config_path = os.path.join(lora_save_path, "adapter_config.json")
                        ...
                        log_with_rank(
                            f"✅ LoRA adapter saved to: {lora_save_path}",
                            rank=dist.get_rank(),
                            logger=logger,
                            log_only_rank_0=True,
                        )
...
class CriticWorker(Worker, DistProfilerExtension):
    ...
    def _build_critic_model_optimizer(self, config):
        ...
        if self._is_lora:
            ...
            # Add missing method for TokenClassification models to avoid PEFT init errors
            if not hasattr(critic_module, 'prepare_inputs_for_generation'):
                def dummy_prepare_inputs(*args, **kwargs):
                    return {}
                critic_module.prepare_inputs_for_generation = dummy_prepare_inputs
